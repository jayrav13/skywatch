...
RSpec.describe Skywatch::Briefer::Models::Taf do
  ...
  describe '.from_awc' do
    ...
    it 'builds position' do
      expect(taf.position).to eq(Skywatch::Shared::Position.new(lat: 41.25407, lon: -70.05892))
    end

    it 'parses forecast groups' do
      expect(taf.forecast_groups.size).to eq(3)
      expect(taf.forecast_groups).to all(be_a(Skywatch::Briefer::Models::TafGroup))
    end
