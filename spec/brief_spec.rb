...
RSpec.describe Skywatch, '.brief' do
  ...
  it 'composes a Brief for an airport' do
    expect(composer).to receive(:compose).with(airport: 'KCDW', at: nil, departing_at: nil).and_return(brief)
    expect(described_class.brief(airport: 'KCDW')).to be(brief)
  end

  it 'composes a Brief for a coordinate pair' do
    expect(composer).to receive(:compose).with(airport: nil, at: [40.688, -74.174], departing_at: nil).and_return(brief)
  ...
  it 'passes departing_at through to composer' do
    ...
    expect(composer).to receive(:compose).with(airport: 'KCDW', at: nil, departing_at: etd).and_return(brief)
